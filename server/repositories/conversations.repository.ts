 ...
 * Acesso direto ao PostgreSQL para conversas, mensagens e perfis.
 * Sem lógica de negócio — apenas queries.
 */
import { query, queryOne, execute } from '../config/db';
import { Conversa, Mensagem, PerfilPet, DashboardStats } from '../types';
import { nomeInstancia } from '../utils/formatters';

// ── Lista de contatos (visão da coluna esquerda) ───────────────
export async function listarConversas(): Promise<any[]> {
  return query(`
    SELECT
      c.id            AS conversa_id,
      c.client_id,
      ...
      c.criado_em,
      -- Novos campos de memória operacional
      c.temperatura_lead,
      c.objecao_principal,
      c.plano_recomendado,
      c.intensidade,
      ...
      c.clinicas_20km,
      c.clinicas_40km,
      -- Total de mensagens (para lógica de fila)
      (SELECT COUNT(*) FROM mensagens m WHERE m.conversa_id = c.id)::int AS total_msgs,
      -- Última mensagem (preview e data no card)
      ...
      END AS chip,
      -- Próximo agendamento (followup pendente)
...
// ── Conversa completa (ao abrir um contato) ───────────────────
export async function buscarConversa(id: string): Promise<any | null> {
  return queryOne(`
    SELECT
      c.*,
      c.numero_externo AS phone,
      ...
      a.slug   AS agente_slug,
      a.nome   AS agente_nome,
      -- Total de mensagens para exibir no painel de perfil
      (SELECT COUNT(*) FROM mensagens m WHERE m.conversa_id = c.id)::int AS total_msgs,
      -- Custo IA acumulado da conversa (Brain + Decisor + Apresentação-planos)
      -- 21/04/2026 — Getúlio: exibir quanto cada conversa está custando
...
// ── Perfil do lead (painel direito) ───────────────────────────
export async function buscarPerfil(clientId: string): Promise<any | null> {
  // Busca dados do cliente + pet mais recente
  ...
  return { ...(cliente || {}), ...(pet || {}) } as any;
}

// Limite de msgs do dashboard — lê da BD, cache 1min
let _msgLimiteCache = 50;
let _msgLimiteTs    = 0;
async function getMsgLimite(): Promise<number> {
  if (Date.now() - _msgLimiteTs > 60000) {
    const r = await queryOne<any>('SELECT valor FROM mari_config WHERE chave=$1', ['dashboard_msgs_limite']).catch(() => null);
    if (r?.valor) _msgLimiteCache = parseInt(r.valor, 10) || 50;
    _msgLimiteTs = Date.now();
  }
  return _msgLimiteCache;
}

// ── Histórico de mensagens paginado ───────────────────────────
export async function buscarMensagens(conversaId: string, limite?: number, antesId?: string): Promise<Mensagem[]> {
  if (!limite) limite = await getMsgLimite();
  if (antesId) {
    return query<Mensagem>(`
      SELECT * FROM mensagens
      WHERE conversa_id = $1 AND id < $2
      ORDER BY timestamp DESC LIMIT $3
    `, [conversaId, antesId, limite]).then(rows => rows.reverse());
  }
  return query<Mensagem>(`
    SELECT * FROM mensagens
    WHERE conversa_id = $1
    ORDER BY timestamp DESC LIMIT $2
  `, [conversaId, limite]).then(rows => rows.reverse());
}
...
export async function buscarStats(): Promise<DashboardStats> {
  ...
  // Queries de hoje + mês (9 agregados em paralelo)
  // `clientes_hoje/mes` usa criado_em — clientes NOVOS, não atividade de retorno.
  // `fechamentos_mes` usa etapa IN ('venda_fechada','pago') — venda realmente concluída.
  const [cl, ms, cu, cm, ca, clm, msm, fecReal, fecPago] = await Promise.all([
    // Clientes novos hoje (criado_em, não ultima_interacao)
    queryOne<any>(`SELECT COUNT(DISTINCT c.client_id) AS n FROM conversas c WHERE DATE(c.criado_em) = $1`, [hoje]),
    // Mensagens hoje
    queryOne<any>(`SELECT COUNT(*) AS n FROM mensagens WHERE DATE(timestamp) = $1`, [hoje]),
    // Custo IA hoje
    queryOne<any>(`SELECT COALESCE(SUM(custo_usd),0) AS n FROM custos_ia WHERE DATE(timestamp) = $1`, [hoje]),
    // Custo IA mês
    queryOne<any>(`SELECT COALESCE(SUM(custo_usd),0) AS n FROM custos_ia WHERE TO_CHAR(timestamp,'YYYY-MM') = $1`, [mes]),
    // Conversas ativas agora
    queryOne<any>(`SELECT COUNT(*) AS n FROM conversas WHERE status = 'ativa'`),
    // Clientes novos no mês (criado_em)
    queryOne<any>(`SELECT COUNT(DISTINCT c.client_id) AS n FROM conversas c WHERE TO_CHAR(c.criado_em,'YYYY-MM') = $1`, [mes]),
    // Mensagens no mês
    queryOne<any>(`SELECT COUNT(*) AS n FROM mensagens WHERE TO_CHAR(timestamp,'YYYY-MM') = $1`, [mes]),
    // Vendas fechadas no mês — etapa venda_fechada (lead confirmado)
    queryOne<any>(`SELECT COUNT(*) AS n FROM conversas WHERE etapa = 'venda_fechada' AND TO_CHAR(ultima_interacao,'YYYY-MM') = $1`, [mes]).catch(() => ({ n: '0' })),
    // Vendas pagas no mês — etapa pago (adesão confirmada no ERP)
    queryOne<any>(`SELECT COUNT(*) AS n FROM conversas WHERE etapa = 'pago' AND TO_CHAR(ultima_interacao,'YYYY-MM') = $1`, [mes]).catch(() => ({ n: '0' })),
  ]);

  const vendas_fechadas = parseInt(fecReal?.n || '0');
  const vendas_pagas    = parseInt(fecPago?.n || '0');

  return {
    clientes_hoje:     parseInt(cl?.n   || '0'),
    msgs_hoje:         parseInt(ms?.n   || '0'),
    custo_hoje:        parseFloat(cu?.n || '0').toFixed(4),
    custo_mes:         parseFloat(cm?.n || '0').toFixed(2),
    conversas_ativas:  parseInt(ca?.n   || '0'),
    clientes_mes:      parseInt(clm?.n  || '0'),
    msgs_mes:          parseInt(msm?.n  || '0'),
    // Soma vendas_fechadas + vendas_pagas — representa vendas concluídas no mês
    fechamentos_mes:   vendas_fechadas + vendas_pagas,
    fechamentos_fonte: 'etapa_venda_fechada_pago',
  };
}

// ── Agendamentos ativos de uma conversa ───────────────────────
...
export async function excluirContato(conversaId: string): Promise<void> {
  ...
  await execute('DELETE FROM identificadores_cliente WHERE client_id = $1', [clientId]);
...
// ── Arquivos Obsidian carregados na última decisão ─────────────
