...
async function identificarEstado(conversaId) {
  ...
  // [CAMADA 1 — lock 22/04/2026, caso Alan]
  // Só classifica um estado "pós-algo" se o cliente já respondeu algo
  // depois que a Mari mandou a mensagem relevante. Antes, a própria fala da
  // Mari (ex.: citar "R$59,99" na abertura) disparava POS_PRECO e o reengajamento
  // soava como se o cliente tivesse recebido proposta — gerando mensagens B2B
  // fora de contexto (caso Alan 22/04/2026 12:09).
  const turnosCliente = await db.one(
    `SELECT COUNT(*)::int AS n FROM mensagens WHERE conversa_id=$1 AND role='user'`,
    [conversaId]
  ).catch(() => ({ n: 0 }));
  const clienteJaRespondeu = (turnosCliente?.n || 0) >= 2;

  if (clienteJaRespondeu) {
    ...
    // POS_PRECO: só se o CLIENTE mencionou preço/valor. A fala da Mari não conta.
    const ultimasCliente = await db.query(
      `SELECT conteudo FROM mensagens WHERE conversa_id=$1 AND role='user' ORDER BY timestamp DESC LIMIT 5`,
      [conversaId]
    ).catch(() => []);
    const textosCliente = ultimasCliente.map(m => (m.conteudo || '').toLowerCase()).join(' ');
    if (/r\$|preço|preco|valor|custa|caro|barato|quanto/.test(textosCliente)) return ESTADOS.POS_PRECO;
  }

  const perfil = await db.one(
    'SELECT * FROM perfil_pet WHERE conversa_id=$1 LIMIT 1', [conversaId]
  ).catch(() => null);
  if (clienteJaRespondeu && (perfil?.especie || perfil?.idade_anos))       return ESTADOS.QUALIFICACAO_INICIAL;

  // Fallback: conversa sem resposta do cliente ainda → estado INICIO
  // (reengajamento usa prompt "suave" sem assumir pós-nada)
...
// [CAMADA 2 lock 22/04/2026] Prompt reforçado com nicho + histórico real pra
// impedir alucinações B2B como a do caso Alan ("estratégia pra vender mais").
async function gerarMensagemVazio(estado, conversaId, sequencia, angulo) {
  ...
  // Últimas 3 mensagens reais da conversa (contexto forte vs resumo)
  const ultimas = await db.query(
    `SELECT role, conteudo FROM mensagens WHERE conversa_id=$1 ORDER BY timestamp DESC LIMIT 3`,
    [conversaId]
  ).catch(() => []);
  const historicoReal = ultimas.reverse().map(m => {
    const autor = m.role === 'user' ? 'CLIENTE' : 'MARI';
    return `${autor}: ${(m.conteudo || '').slice(0, 200)}`;
  }).join('\n');

  const system = [
    'Você é a Mari, consultora da Plamev — empresa de PLANO DE SAÚDE PARA PETS (cães e gatos).',
    'Você conversa com TUTORES de animais de estimação sobre consultas, exames, cirurgias, vacinas e castração de pet.',
    '',
    '⚠️ PROIBIDO ABSOLUTO nesta mensagem:',
    '- Falar de estratégia de vendas, marketing, consultoria, B2B, crescimento de empresa, metas corporativas',
    '- Fazer perguntas genéricas tipo "qual seu maior desafio?" sem conexão com o pet',
    '- Usar linguagem coach/consultor de negócios',
    '- Inventar contexto que não está no histórico real',
    '- Mencionar produtos/serviços que não sejam plano de saúde pet Plamev',
    '',
    'REGRAS DE ESTILO:',
    '- 1 ou 2 frases curtas, linguagem natural brasileira de WhatsApp',
    '- 1 pergunta ou CTA leve no final, SEMPRE conectado ao pet do cliente',
    '- Sem travessão (—), sem duplo-hífen (--), sem **negrito duplo** nem ~~tachado duplo~~',
    '- Se o cliente ainda não deu informações (conversa no início), não faça pergunta específica — use tom suave de reabertura',
    '- Não mencione que você tentou contato antes',
    '',
    'Responda APENAS com a mensagem final (sem aspas, sem explicação antes ou depois).',
  ].join('\n');

  const prompt = [
    `Situação: reabrir conversa com tutor de pet que não respondeu.`,
    `Tentativa ${(sequencia || 0) + 1} de 8.`,
    `Tom desta tentativa: ${instrucaoTom}`,
    `Intenção: ${intencao}`,
    `Horário: ${hora < 12 ? 'manhã' : hora < 18 ? 'tarde' : 'noite'}`,
    '',
    'Contexto do lead:',
    nome    ? `- Tutor: ${nome}` : '- Tutor: (nome não informado)',
    nomePet ? `- Pet: ${nomePet}${raca ? ' (' + raca + ')' : ''}` : '- Pet: (não informado ainda)',
    plano   ? `- Plano discutido: ${plano}` : '',
    objecao ? `- Objeção anterior: ${objecao}` : '',
    resumo  ? `- Resumo: ${resumo}` : '',
    '',
    historicoReal
      ? `Últimas mensagens REAIS da conversa:\n${historicoReal}`
      : '(conversa ainda sem mensagens — tom inicial suave)',
    '',
    'Escreva a mensagem de reabertura agora, seguindo TODAS as regras do system.',
  ].filter(Boolean).join('\n');
...
async function agendarVacuo(conversaId, sequenciaAtual = 0) {
  ...
  await db.run(
    `DELETE FROM followup_agendado WHERE conversa_id=$1 AND status='pendente' AND tipo='vacuo'`,
    [conversaId]
  ).catch(() => {});
...
async function executarPendentes() {
  ...
  for (const row of pendentes) {
    ...
    if ((Date.now() - new Date(row.ultima_interacao).getTime()) / 60000 < 15) {
      await db.run('DELETE FROM followup_agendado WHERE id=$1', [row.id]);
      continue;
    }
    ...
    // ── CAMADA 3 · Guard Rail pós-Haiku (lock 22/04/2026, caso Alan) ────────
    // Valida a mensagem de reengajamento ANTES de enviar. Se detectar linguagem
    // fora do nicho (B2B, coach, estratégia corporativa), bloqueia o envio,
    // marca como 'ignorado_validador' e loga pra análise.
    try {
      const validadorReeng = require('./validador-reengajamento');
      const v = validadorReeng.validar(row.mensagem);
      if (!v.ok) {
        await db.run(
          `UPDATE followup_agendado
             SET status='ignorado_validador',
                 executado_em=NOW(),
                 motivo_cliente=COALESCE(motivo_cliente,'') || $2
           WHERE id=$1`,
          [row.id, ` [bloqueio:${v.categoria}${v.termo ? '/'+v.termo : ''}]`]
        ).catch(() => {});
        console.warn(`[REENG-GUARD] 🛑 BLOQUEADO ${row.numero_externo} [${v.categoria}] termo:"${v.termo || '-'}" msg:"${(row.mensagem || '').slice(0,80)}"`);
        continue; // não envia
      }
    } catch (e) {
      console.warn('[REENG-GUARD] validador indisponível, seguindo:', e.message);
    }
    ...
    .then(async ok => {
      ...
      if (ok) {
        ...
        // ⚠️ Fix 20/04/2026 — 'ia_reengajamento' violava CHECK constraint de
        // mensagens.enviado_por (só aceita ia/humano/supervisora), então o INSERT
        // falhava silenciosamente e reengajamentos não apareciam no dashboard.
        // Agora persiste como 'ia' com marcador no metadata pra rastreabilidade.
        await db.salvarMensagem(
          row.conversa_id, 'agent', row.mensagem, 'ia', null, null
        ).catch(e => console.error('[REENG] salvarMensagem erro:', e.message));

        // ── CAMADA 4 · Juiz de alucinação (fire-and-forget, auditoria) ────
        // Não bloqueia envio — registra em alucinacoes_mari pra revisão na
        // página "Alucinações" do Intelligence V1 se detectar problema.
        try {
          const juiz = require('./juiz');
          juiz.avaliarEmBackground({
            conversaId: row.conversa_id,
            phone: row.numero_externo,
            mensagemCliente: '(reengajamento automático — sem msg do cliente)',
            respostaMari: row.mensagem,
            contextoEnviado: `reengajamento: estado=${row.motivo || '?'} ângulo=${row.angulo || '?'} seq=${row.sequencia || '?'}`,
          });
        } catch (e) { /* juiz opcional */ }
        // Adiciona metadata indicando que foi reengajamento (pra dashboard filtrar)
        const metaJson = JSON.stringify({ origem: 'reengajamento', tipo: row.motivo || 'reengajamento' });
        await db.run(
          `UPDATE mensagens
           SET metadata = COALESCE(metadata,'{}'::jsonb) || $1::jsonb
           WHERE id = (
             SELECT id FROM mensagens
             WHERE conversa_id=$2 AND conteudo=$3 AND role='agent'
             ORDER BY timestamp DESC LIMIT 1
           )`,
          [metaJson, row.conversa_id, row.mensagem]
        ).catch(() => {});
