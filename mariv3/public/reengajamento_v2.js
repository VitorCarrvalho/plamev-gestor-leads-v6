...
async function gerarMensagemVazio(estado, conversaId, sequencia, angulo) {
  ...
  const perfil  = conv ? await db.buscarOuCriarPerfil?.(conv.client_id).catch(() => null) : null;
...
- Horário: ${hora < 12 ? 'manhã' : hora < 18 ? 'tarde' : 'noite'}`;
  ...
  const modelo = await db.getConfig?.('modelo_reengajamento', 'claude-haiku-4-5').catch(() => 'claude-haiku-4-5') || 'claude-haiku-4-5';
