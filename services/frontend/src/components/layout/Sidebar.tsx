...
export const Sidebar: React.FC<SidebarProps> = ({ pilar, subPage, onNavigate, onLogout, onPerfilClick, userEmail, userName, userFoto, badgeCounts = {}, open, onClose }) => {
  ...
  return (
    ...
    <div className="border-t border-slate-800 p-3">
      <button
        ...
        className="w-full flex items-center gap-3 px-2 py-2 rounded-lg mb-2 hover:bg-slate-800/60 transition-colors text-left group"
        title="Editar perfil"
      >
        <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
          {userFoto
        ...
        <div className="flex flex-col min-w-0 flex-1">
          <span className="text-xs font-medium text-slate-200 truncate group-hover:text-slate-100">{userName || 'Supervisor'}</span>
