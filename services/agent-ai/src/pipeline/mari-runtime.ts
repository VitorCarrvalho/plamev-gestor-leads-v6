...
export function formatProductCatalogPrompt(rows: CatalogRow[]) {
  ...
  const lines = [
    '# DADOS DO PRODUTO',
    'Use APENAS os valores abaixo. NUNCA invente planos ou valores fora desta tabela.',
    'O PREÇO A DIVULGAR é o "preço vigente". O PISO ABSOLUTO é o menor valor que pode ser mencionado ao cliente — nunca ofereça abaixo dele.',
  ];
  ...
  for (const [slug, plan] of grouped.entries()) {
    ...
    for (const modalidade of plan.modalidades) {
      ...
      const oferta  = modalidade.valor_oferta;
      const limite  = modalidade.valor_limite;
      ...
      const parts: string[] = [`- ${modalidade.modalidade} — vigente: ${priceStr}`];
      if (oferta  != null) parts.push(`oferta-WOW: ${formatCurrency(oferta)}/mês`);
      if (limite  != null) parts.push(`⛔ PISO ABSOLUTO: ${formatCurrency(limite)} (nunca oferecer abaixo)`);
