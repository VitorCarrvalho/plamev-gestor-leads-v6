...
router.get('/', async (req, res) => {
  try {
    ...
    const rows = await query<any>(
      `SELECT * FROM dashv5_audit_log ${where} ORDER BY criado_em DESC LIMIT ${lim}`, vals
    );
...
router.get('/acoes', async (_req, res) => {
  try {
    const rows = await query<any>(
      `SELECT acao, COUNT(*)::int AS n FROM dashv5_audit_log GROUP BY acao ORDER BY n DESC`
    );
