...
export async function buscarMensagens(orgId: string, conversaId: string, limite?: number, antesId?: string): Promise<Mensagem[]> {
  ...
  if (antesId) {
    ...
    `, [conversaId, antesId, limite, orgId]).then(rows => rows.reverse());
  }
  return query<Mensagem>(`
    SELECT m.* FROM mensagens m
    JOIN conversas c ON c.id = m.conversa_id
  ...
  `, [conversaId, limite, orgId]).then(rows => rows.reverse());
}

// ── Stats do header ───────────────────────────────────────────
export async function buscarStats(orgId: string): Promise<DashboardStats> {
